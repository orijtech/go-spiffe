...
import (
	...
	spiffe "github.com/orijtech/go-spiffe"
)
...
func TestFURINamesFromPEM(t *testing.T) {
	uris, err := spiffe.FuriNamesFromPEM(strings.NewReader(goodCert))
	if err != nil {
		t.Error(err)
	}
	...
	uris, err = spiffe.FuriNamesFromPEM(strings.NewReader(badCert))
	if err == nil {
		t.Fatal("Expected to fail")
	}
